...
Qobuz requires a user token and app credentials for playback.
...
5. In the request URL parameters, find `app_id`.
6. For `app_secret`, inspect the Qobuz JavaScript bundle and search for the secret key.
7. Configure:
   ```toml
   [sources.qobuz]
   enabled = true
...
Pandora requires a CSRF token for search.
